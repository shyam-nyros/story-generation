...
const headers = {
  ...
  Authorization: `Bearer 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`,
...
async function getCompletions() {
  ...
  try {
    ...
    if (aresponseData.error) {
      throw aresponseData.error;
    }
    ...
    const imgData = {
      prompt:
        image_prompt.value.replace(/\?/g, "") + " in a photorealistic style",
      n: 5,
    ...
    const carousel_data = storyData
      .map((e, i) => {
        const image_url =
          "data:image/png;base64," + responseData.data[i]?.b64_json;
        return `
        <div class="page">
